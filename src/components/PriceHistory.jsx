...
export default function PriceHistory({ history, loading }) {
  ...
  if (loading) {
    return (
      <div className="bg-card rounded-2xl p-4">
        <h2 className="text-content font-semibold mb-4">Historial de precio</h2>
  ...
  if (!chart) {
    ...
    return (
      <div className="bg-card rounded-2xl p-4">
        <h2 className="text-content font-semibold mb-2">Historial de precio</h2>
        <p className="text-content-dim text-sm">
          Aún no hay historial suficiente para este producto.
        </p>
        <p className="text-content-faint text-xs mt-1.5">
          {registrados === 1
  ...
  return (
    <div className="bg-card rounded-2xl p-4">
      <div className="flex items-baseline justify-between mb-3">
        <h2 className="text-content font-semibold">Historial de precio</h2>
        <span className="text-content-faint text-xs">{history.length} registros</span>
      </div>
      ...
      <div className="flex justify-between mt-1.5">
        <span className="text-content-faint text-[10px]">{label(first.date)}</span>
        <span className="text-content-faint text-[10px]">{label(last.date)}</span>
      </div>

      <div className="flex justify-between mt-3 pt-3 border-t border-line">
        <div>
          <p className="text-content-faint text-[10px]">Mínimo</p>
          <p className="text-green-400 text-sm font-semibold">{formatPrice(min)}</p>
        </div>
        <div className="text-center">
          <p className="text-content-faint text-[10px]">Máximo</p>
          <p className="text-content text-sm font-semibold">{formatPrice(max)}</p>
        </div>
        <div className="text-right">
          <p className="text-content-faint text-[10px]">Desde {label(first.date)}</p>
          <p className={`text-sm font-semibold ${delta < 0 ? 'text-green-400' : delta > 0 ? 'text-orange-400' : 'text-content-soft'}`}>
