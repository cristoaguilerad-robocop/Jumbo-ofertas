...
export default function Search() {
  ...
  return (
    <div className="min-h-screen bg-canvas pb-nav">
      ...
      {/* Header */}
      <div className="bg-surface px-4 pt-header pb-4 sticky top-0 z-40">
        <div className="max-w-lg mx-auto space-y-3">
          <div className="flex gap-2">
            <div className="flex-1 relative">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"
                className="w-4 h-4 text-content-dim absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none">
              ...
              <input
                ...
                className="w-full bg-card text-content placeholder-gray-500 rounded-xl pl-9 pr-4 py-3 text-sm outline-none focus:ring-2 focus:ring-green-500"
              />
              {(loading || scanLoading) && (
              ...
              {query && !loading && (
                <button onClick={() => setQuery('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-content-dim">
            ...
            <button
              ...
              className="bg-card hover:bg-muted text-content-soft rounded-xl px-3 flex items-center justify-center"
            >
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${
                onlyOffers ? 'bg-orange-500 text-white' : 'bg-card text-content-dim'
              }`}
            >
              🏷️ Solo ofertas
            </button>
            ...
            {categories.map(cat => (
              <button
                ...
                className={`shrink-0 px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${
                  category === cat ? 'bg-green-500 text-white' : 'bg-card text-content-dim'
                }`}
              >
                {cat}
              </button>
      ...
      <div className="max-w-lg mx-auto px-4 pt-4 space-y-2">
        ...
        {scanned && (
          <div className="bg-green-500/10 border border-green-500/20 rounded-xl p-3 flex items-center gap-3">
            {scanned.imageUrl && (
              <img {...productImageProps(scanned.imageUrl, 64)} alt="" className="w-16 h-16 rounded-xl object-contain product-media p-1 shrink-0" />
            )}
            <div className="flex-1 min-w-0">
              <p className="text-green-400 text-sm font-medium">Agregado a tu lista</p>
              <p className="text-content-soft text-xs truncate">{scanned.name}</p>
            </div>
            <button
        ...
        {pendingBarcode && (
          <div className="bg-green-500/10 border border-green-500/20 rounded-xl p-3 space-y-2">
            ...
            {suggestion?.loading && (
              <p className="text-xs text-content-dim">Identificando el código…</p>
            )}

            {suggestion && !suggestion.loading && suggestion.hint && (
              <p className="text-xs text-content-soft">
                Parece ser <span className="text-content font-medium">
                  {[suggestion.hint.brand, suggestion.hint.name, suggestion.hint.quantity]
                    .filter(Boolean).join(' ')}
                </span>
                <span className="text-content-faint"> · según Open Food Facts</span>
              </p>
            )}
              ...
              <div className="space-y-1.5 pt-1">
                {suggestion.candidates.map(c => (
                  <button
                    ...
                    className="w-full bg-card hover:bg-muted rounded-xl p-2.5 flex items-center gap-2.5 text-left transition-colors"
                  >
                    {c.imageUrl && (
                      <img {...productImageProps(c.imageUrl, 64)} alt="" loading="lazy" className="w-16 h-16 rounded-xl object-contain product-media p-1 shrink-0" />
                    )}
                    <span className="flex-1 min-w-0">
                      <span className="block text-content text-sm leading-snug line-clamp-2">{c.name}</span>
                      <span className="block text-content-dim text-[11px] mt-0.5">
            ...
            <button
              ...
              className="text-xs text-content-dim underline"
            >
        ...
        {scanLoading && (
          <div className="flex items-center justify-center py-10 gap-3">
            ...
            <p className="text-content-dim text-sm">Buscando en Jumbo...</p>
          </div>
        )}

        {!scanLoading && error && (
          <div className="bg-card border border-line rounded-xl p-3 text-xs text-content-dim">
        ...
        {!scanLoading && results.length > 0 && (
          <>
            <p className="text-content-faint text-xs px-1">
            ...
            {hasMore && (
              <button
                ...
                className="w-full py-3 rounded-xl bg-card hover:bg-muted text-content-soft text-sm font-medium transition-colors disabled:opacity-60"
              >
